...
codigo, listado = pedir("GET", "/api-queries/users")
...
while time.time() < limite:
    codigo, actual = pedir("GET", "/api-queries/orders/" + str(orden["id"]))
...
if not orden2:
    ...
    estado2 = None
    while time.time() < limite:
        codigo, actual = pedir("GET", "/api-queries/orders/" + str(orden2["id"]))
...
print("RESULTADO: TODO OK — el flujo asincrono completo funciona")
sys.exit(0)
